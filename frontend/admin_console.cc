...
int parse_backend_servers(char* buf){
	...
	while(found2!=std::string::npos){
		...
		string ip_addr = data.substr(found1 + 1, found2 - 1);
		...
		}else server.backend_type = 3;

		frontend_servers[id++] = server;

		std::size_t found1 = found3+1;
		std::size_t found2 = data.find_first_of(":", found2 + 1);
		std::size_t found3 = data.find_first_of(",", found2 + 1);
...
int main(int argc, char *argv[])
{
	...
	while(true){
		...
		string contact_master = "U" + backend_servers[nodeID].ip + ":" + to_string(backend_servers[nodeID].port);
		sendto(udp_fd, contact_master.c_str(), contact_master.size(), 0,
				(struct sockaddr *)&dest, sizeof(dest));
		...
		while (true){
			...
			if (reply.compare("y") == 0){
				...
				inet_pton(AF_INET, backend_servers[nodeID].ip.c_str(), &(backend.sin_addr));
...
//				strcat(m, ",email");
			...
			}else if (reply.compare("n") ==0) continue;
			else {
	...
	//	bind(frontend_fd, (struct sockaddr*) &frontend_servaddr, sizeof(frontend_servaddr));
	//
	...
	//	bind(backend_fd, (struct sockaddr*) &backend_servaddr, sizeof(backend_servaddr));
	//
